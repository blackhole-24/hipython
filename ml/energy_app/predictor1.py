"""
predictor.py — 전력 피크 예측 추론 모듈 (v4)
────────────────────────────────────────────────────────────────
...
[필요 파일 — 같은 폴더에 있어야 함]
    energy_pipeline_v4.pkl  : 학습된 모델
    PowerMgt.db             : DB (predict_from_db 사용 시)

[팀원 전달 정보]
    파일명      : predictor.py
    예측함수    : predict(df)
    pkl 파일    : energy_pipeline_v4.pkl
    입력 컬럼   : Date, hour, temperature, humidity, windspeed,
                  rainfall, op_code, output, weekday, weekend, holiday
    출력 컬럼   : y1=15분, y2=30분, y3=45분, y4=60분 (kW)
────────────────────────────────────────────────────────────────
"""
...
_PKL_PATH = os.path.join(_BASE_DIR, 'energy_pipeline_v4.pkl')
...
# 생산구분별 공장인원 평균 (훈련 데이터 통계)
_MANPOWER = {0: 0.0, 1: 2.13, 2: 0.84, 3: 1.43, 4: 6.34}


def _tariff(month: int) -> float:
    """계절별 전기요금 단가 (원/kWh)"""
...
def _tou(month: int, hour: int, is_holiday: int, is_weekend: int) -> tuple:
    """(tou_bucket, tou_price) — TOU 요금 구간"""
    if is_holiday or is_weekend:
        return 0, 95.7
    if hour >= 22 or hour <= 7:
        return 0, 95.7
    if month in [6, 7, 8]:
        return (2, 155.0) if hour in range(10, 18) else (1, 121.5)
    if month in [11, 12, 1, 2]:
        return (2, 155.0) if hour in [9, 10, 17, 18, 19] else (1, 121.5)
    return 1, 121.5
...
def predict_from_db(db_path: str, date: str) -> dict:
    ...
    Parameters
    ----------
    db_path : str  PowerMgt.db 경로 (예: 'db/PowerMgt.db')
    date    : str  'YYYY-MM-DD'
    """
    import sqlite3
